...
const URL = 'http://localhost:8082/maestria/Laboratorio';
...
async function getContent(){
  const html = await fetch(URL + '/expenses/create').then(res => res.text());
  return html;
}
...
google.charts.load('current', {'packages':['bar']});
      ...
      async function drawChart() {
        ...
        const http = await fetch(URL + '/expenses/getExpensesJSON')
        .then(json => json.json())
        .then(res => res);
