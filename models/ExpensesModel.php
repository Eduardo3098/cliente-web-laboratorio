...
class ExpensesModel extends Model implements iModel {
    ...
    public function getByUserIdAndLimit($userid, $n){
        $items = [];
        try{
            error_log('ExpensesModel::getByUserIdAndLimit(): $userid ->' .$userid);
            error_log('ExpensesModel::getByUserIdAndLimit(): $n ->' .$n);
    ...
    function getTotalAmountThisMonth($iduser){
        try{
            $year = date('Y');
            $month = date('m');
            $query = $this->prepare('SELECT SUM(amount) AS total FROM expenses WHERE YEAR(date) = :year AND MONTH(date) = :month AND id_user = :iduser');
    ...
    function getMaxExpensesThisMonth($iduser){
        try{
            $year = date('Y');
            $month = date('m');
            $query = $this->prepare('SELECT MAX(amount) AS total FROM expenses WHERE YEAR(date) = :year AND MONTH(date) = :month AND id_user = :iduser');
            $query->execute(['year' => $year, 'month' => $month, 'iduser' => $iduser]);
